...
class Trader:
    ...
    def tradeOsmium(
        ...
        if directionalGap >= 1.5:
            buyTakeEdge -= 0.5
        elif directionalGap <= -1.5:
            sellTakeEdge -= 0.5

        buyTakeEdge = max(1.5, buyTakeEdge)
        sellTakeEdge = max(1.5, sellTakeEdge)

        for askPrice, askVolume in self.sortedAsks(orderDepth):
            available = -askVolume
            if askPrice <= adjustedFair - buyTakeEdge:
                qty = min(available, self.maxBuyCapacity(product, position, buyOrderVolume))
                if qty > 0:
                    orders.append(Order(product, askPrice, qty))
                    buyOrderVolume += qty
                    takenBuys.append((askPrice, qty))
        ...
        for bidPrice, bidVolume in self.sortedBids(orderDepth):
            ...
            if bidPrice >= adjustedFair + sellTakeEdge:
                qty = min(available, self.maxSellCapacity(product, position, sellOrderVolume))
                if qty > 0:
                    orders.append(Order(product, bidPrice, -qty))
                    sellOrderVolume += qty
                    takenSells.append((bidPrice, qty))
        ...
        if positionAfterTake > 36:
            for bidPrice, bidVolume in self.sortedBids(orderDepth):
                if bidPrice >= adjustedFair - params["clear_edge"]:
                    qty = min(bidVolume, positionAfterTake - 36, self.maxSellCapacity(product, position, sellOrderVolume))
                    if qty > 0:
                        orders.append(Order(product, bidPrice, -qty))
                        sellOrderVolume += qty
                        positionAfterTake -= qty
        ...
        elif positionAfterTake < -36:
            for askPrice, askVolume in self.sortedAsks(orderDepth):
                if askPrice <= adjustedFair + params["clear_edge"]:
                    qty = min(-askVolume, (-36) - positionAfterTake, self.maxBuyCapacity(product, position, buyOrderVolume))
                    if qty > 0:
                        orders.append(Order(product, askPrice, qty))
                        buyOrderVolume += qty
                        positionAfterTake += qty
        ...
        if abs(directionalGap) < 0.75:
            buySize = max(4, buySize - 4)
            sellSize = max(4, sellSize - 4)
        elif directionalGap >= 2.0:
            buySize += 4
            sellSize = max(4, sellSize - 3)
        elif directionalGap <= -2.0:
            sellSize += 4
            buySize = max(4, buySize - 3)

        if imbalance >= 0.30:
            buySize += 3
            sellSize = max(4, sellSize - 2)
        elif imbalance <= -0.30:
            sellSize += 3
            buySize = max(4, buySize - 2)

        if positionAfterTake > 12:
            askQuote -= 1
            sellSize += 4
            if directionalGap < 2.5:
                buySize = 0
            else:
                buySize = max(0, buySize - 6)
        ...
        if bidQuote >= askQuote:
            bidQuote = math.floor(adjustedFair - 1)
            askQuote = math.ceil(adjustedFair + 1)
        ...
        if self.shouldLogProduct(product):
            ...
            logger.print(
                ...
                "takenBuys =", takenBuys[:4],
                "takenSells =", takenSells[:4],
                "quotes =", [(int(bidQuote), int(buyQty)), (int(askQuote), -int(sellQty))],
                "finalOrders =", [(order.price, order.quantity) for order in orders[:8]],
            )
